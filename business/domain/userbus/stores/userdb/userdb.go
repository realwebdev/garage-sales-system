...
// NewStore conctructs the api for data access.
...
func (s *Store) Create(ctx context.Context, usr userbus.User) error {
	...
	VALUES
		(:user_id, :name, :email, :password_hash, :roles, :department, :data_created, :data_updated)`
...
func (s *Store) Query(ctx context.Context, filter userbus.QueryFilter, orderBy order.By, page page.Page) ([]userbus.User, error) {
	...
	const q = `
	SELECT
		user_id, name, email, password_hash, roles, department, enabled, date_created, data_updated
	FROM
		user`
	...
	buf.WriteString(" OFFSET :offset ROWS FETX NEXT :rows_per_page ROWS ONLY")
