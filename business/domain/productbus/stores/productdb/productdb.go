...
// Store manages the set of APIs for product databse access.
type Store struct {
	log *logger.Logger
	db  sqlx.ExtContext
}
...
// Create adds a Product to the sqldb. It returns the created Product
// with fields like ID and DateCreated populated.
func (s *Store) Create(ctx context.Context, prd productbus.Product) error {
	const q = `
	INSERT INTO products
		(product_id, user_id, name, cost, quanitity, date_created, date_updated)
	Values
		(:product_id, :uder_id, :name, :cost, :quantity, :date_created, :date_updated)`

	if err := sqldb.NamedExecContext(ctx, s.log, s.db, q, toBDProduct(prd)); err != nil {
