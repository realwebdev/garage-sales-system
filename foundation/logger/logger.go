...
// TraceIDDn represent a function that can return the trace id from
// the specified context.
type TraceIDFn func(ctx context.Context) string

// Logger represnts a logger for logging information
type Logger struct {
	discard    bool
	handler    slog.Handler
	tracerIDFn TraceIDFn
}
...
// Debug logs athe LevelDebug with the given context.
...
// Info logs at the LevelInfo with the given context.
...
// Warn logs at the LevelWarn with the given context.
...
// Error logs at the LevelWarn with the given context.
...
func (log *Logger) write(ctx context.Context, level Level, caller int, msg string, args ...any) {
	...
	if log.tracerIDFn != nil {
		args = append(args, "trace_id", log.tracerIDFn(ctx))
	}
...
func new(w io.Writer, minLevel Level, serviceName string, traceIDFn TraceIDFn, events Events) *Logger {
	// Convert the file name to just the name.ext when this key/value will be logger.
	f := func(group []string, a slog.Attr) slog.Attr {
	...
	attrs := []slog.Attr{
		{Key: "serice", Value: slog.StringValue(serviceName)},
	}
